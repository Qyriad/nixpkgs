...
stdenv.mkDerivation (finalAttrs: {
  ...
  # This version of cppitertools considers itself as having used the default value, and issues
  # warning, unless -Dcppitertools_INSTALL_CMAKE_DIR is present as an environment variable.
  # It doesn't actually use the value from this environment variable, though, so we still
  # need to pass it below.
  env.cppitertools_INSTALL_CMAKE_DIR = finalAttrs.cmakeInstallDir;

  cmakeFlags = [
    "-Dcppitertools_INSTALL_CMAKE_DIR=${finalAttrs.cmakeInstallDir}"
  ];

  prePatch = ''
    # Mark the `.` install target as non-default.
    substituteInPlace CMakeLists.txt \
      --replace-warn "  DIRECTORY ." "  DIRECTORY . EXCLUDE_FROM_ALL"
